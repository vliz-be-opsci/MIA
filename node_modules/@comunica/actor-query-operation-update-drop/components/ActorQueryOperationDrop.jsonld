{
  "@context": [
    "https://linkedsoftwaredependencies.org/bundles/npm/@comunica/actor-query-operation-update-drop/^3.0.0/components/context.jsonld",
    "https://linkedsoftwaredependencies.org/bundles/npm/@comunica/core/^3.0.0/components/context.jsonld",
    "https://linkedsoftwaredependencies.org/bundles/npm/@comunica/bus-query-operation/^3.0.0/components/context.jsonld"
  ],
  "@id": "npmd:@comunica/actor-query-operation-update-drop",
  "components": [
    {
      "@id": "caqoud:components/ActorQueryOperationDrop.jsonld#ActorQueryOperationDrop",
      "@type": "Class",
      "requireElement": "ActorQueryOperationDrop",
      "extends": [
        {
          "@type": "GenericComponentExtension",
          "component": "cbqo:components/ActorQueryOperationTypedMediated.jsonld#ActorQueryOperationTypedMediated",
          "genericTypeInstances": [
            "urn:npm:sparqlalgebrajs:Drop"
          ]
        }
      ],
      "comment": "A [Query Operation](https://github.com/comunica/comunica/tree/master/packages/bus-query-operation) actor that handles SPARQL drop operations.",
      "parameters": [
        {
          "@id": "caqoud:components/ActorQueryOperationDrop.jsonld#ActorQueryOperationDrop_args_mediatorUpdateQuads",
          "range": "cc:components/Mediator.jsonld#Mediator",
          "comment": "The RDF Update Quads mediator"
        },
        {
          "@id": "caqoud:components/ActorQueryOperationDrop.jsonld#ActorQueryOperationDrop_args_mediatorQueryOperation",
          "range": "cc:components/Mediator.jsonld#Mediator"
        },
        {
          "@id": "caqoud:components/ActorQueryOperationDrop.jsonld#ActorQueryOperationDrop_args_name",
          "range": "xsd:string",
          "default": {
            "@id": "rdf:subject"
          },
          "comment": "The name for this actor."
        },
        {
          "@id": "caqoud:components/ActorQueryOperationDrop.jsonld#ActorQueryOperationDrop_args_bus",
          "range": {
            "@type": "ParameterRangeGenericComponent",
            "component": "cc:components/Bus.jsonld#Bus",
            "genericTypeInstances": [
              {
                "@type": "ParameterRangeGenericComponent",
                "component": "cc:components/Actor.jsonld#Actor",
                "genericTypeInstances": [
                  {
                    "@type": "ParameterRangeGenericTypeReference",
                    "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^3.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_I"
                  },
                  {
                    "@type": "ParameterRangeGenericTypeReference",
                    "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^3.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_T"
                  },
                  {
                    "@type": "ParameterRangeGenericTypeReference",
                    "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^3.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_O"
                  }
                ]
              },
              {
                "@type": "ParameterRangeGenericTypeReference",
                "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^3.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_I"
              },
              {
                "@type": "ParameterRangeGenericTypeReference",
                "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^3.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_T"
              },
              {
                "@type": "ParameterRangeGenericTypeReference",
                "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^3.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_O"
              }
            ]
          },
          "default": {
            "@id": "cbqo:components/ActorQueryOperation.jsonld#ActorQueryOperation_default_bus",
            "@type": "cbqo:components/BusQueryOperation.jsonld#BusQueryOperation"
          },
          "comment": "The bus this actor subscribes to."
        },
        {
          "@id": "caqoud:components/ActorQueryOperationDrop.jsonld#ActorQueryOperationDrop_args_beforeActors",
          "range": {
            "@type": "ParameterRangeUnion",
            "parameterRangeElements": [
              {
                "@type": "ParameterRangeArray",
                "parameterRangeValue": {
                  "@type": "ParameterRangeGenericComponent",
                  "component": "cc:components/Actor.jsonld#Actor",
                  "genericTypeInstances": [
                    {
                      "@type": "ParameterRangeGenericTypeReference",
                      "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^3.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_I"
                    },
                    {
                      "@type": "ParameterRangeGenericTypeReference",
                      "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^3.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_T"
                    },
                    {
                      "@type": "ParameterRangeGenericTypeReference",
                      "parameterRangeGenericType": "npmd:@comunica/actor-abstract-mediatyped/^3.0.0/components/ActorAbstractMediaTyped.jsonld#ActorAbstractMediaTyped__generic_O"
                    }
                  ]
                }
              },
              {
                "@type": "ParameterRangeUndefined"
              }
            ]
          },
          "comment": "Actor that must be registered in the bus before this actor."
        }
      ],
      "memberFields": [
        {
          "@id": "caqoud:components/ActorQueryOperationDrop.jsonld#ActorQueryOperationDrop__member_mediatorUpdateQuads",
          "memberFieldName": "mediatorUpdateQuads",
          "range": "cc:components/Mediator.jsonld#Mediator"
        },
        {
          "@id": "caqoud:components/ActorQueryOperationDrop.jsonld#ActorQueryOperationDrop__member_constructor",
          "memberFieldName": "constructor"
        },
        {
          "@id": "caqoud:components/ActorQueryOperationDrop.jsonld#ActorQueryOperationDrop__member_testOperation",
          "memberFieldName": "testOperation"
        },
        {
          "@id": "caqoud:components/ActorQueryOperationDrop.jsonld#ActorQueryOperationDrop__member_runOperation",
          "memberFieldName": "runOperation"
        }
      ],
      "constructorArguments": [
        {
          "@id": "caqoud:components/ActorQueryOperationDrop.jsonld#ActorQueryOperationDrop_args__constructorArgument",
          "fields": [
            {
              "keyRaw": "mediatorUpdateQuads",
              "value": {
                "@id": "caqoud:components/ActorQueryOperationDrop.jsonld#ActorQueryOperationDrop_args_mediatorUpdateQuads"
              }
            },
            {
              "keyRaw": "mediatorQueryOperation",
              "value": {
                "@id": "caqoud:components/ActorQueryOperationDrop.jsonld#ActorQueryOperationDrop_args_mediatorQueryOperation"
              }
            },
            {
              "keyRaw": "name",
              "value": {
                "@id": "caqoud:components/ActorQueryOperationDrop.jsonld#ActorQueryOperationDrop_args_name"
              }
            },
            {
              "keyRaw": "bus",
              "value": {
                "@id": "caqoud:components/ActorQueryOperationDrop.jsonld#ActorQueryOperationDrop_args_bus"
              }
            },
            {
              "keyRaw": "beforeActors",
              "value": {
                "@id": "caqoud:components/ActorQueryOperationDrop.jsonld#ActorQueryOperationDrop_args_beforeActors"
              }
            }
          ]
        }
      ]
    },
    {
      "@id": "caqoud:components/ActorQueryOperationDrop.jsonld#IActorQueryOperationDropArgs",
      "@type": "AbstractClass",
      "requireElement": "IActorQueryOperationDropArgs",
      "extends": [
        "cbqo:components/ActorQueryOperationTypedMediated.jsonld#IActorQueryOperationTypedMediatedArgs"
      ],
      "parameters": [],
      "memberFields": [
        {
          "@id": "caqoud:components/ActorQueryOperationDrop.jsonld#IActorQueryOperationDropArgs__member_mediatorUpdateQuads",
          "memberFieldName": "mediatorUpdateQuads"
        }
      ],
      "constructorArguments": []
    }
  ]
}