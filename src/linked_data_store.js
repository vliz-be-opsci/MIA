...
export async function getLinkedDataNQuads(uri) {
  ...
  try {
    // get data from uri
    const data = await fetchData(uri, return_formats);
  } catch (error) {
    console.log(error);
  }
}

export async function getLinkedDataNQuadsOld(uri) {
    ...
    switch (data.format) {
      case "application/ld+json":
        try {
          ...
          const nquads = await jsonld.toRDF(jsontext, {
          ...
          for (const link of links) {
            ...
            if (type === "application/ld+json") {
              ...
              all_parsed_responses =
                all_parsed_responses.concat(parsed_response);
            } else {
...
async function fetchData(fetcher, uri, formats) {
  for (const format of formats) {
    try {
      ...
      if (response.ok && contentType.includes(format)) {
        fetcher.load(uri, { headers: { Accept: format } });
        return;
      }
    } catch (error) {
      console.log(error);
